...
// Import dan gunakan route adminCustomerMapping
const adminCustomerMappingRouter = require('./routes/adminCustomerMapping');
app.use('/admin/customer-mapping', adminAuth, adminCustomerMappingRouter);
...
try {
    ...
    // Inisialisasi RADIUS server jika diaktifkan di settings.json
    const { radiusServer } = require('./config/radius');
    const radiusConfig = getSetting('radius', {});
    if (radiusConfig.enabled || getSetting('user_auth_mode') === 'radius') {
        radiusServer.initialize().then(() => {
            return radiusServer.startAuthServer();
        }).then(started => {
            if (started) {
                logger.info(`RADIUS Server auto-started on port ${radiusServer.port}`);
            } else {
                logger.error('Failed to auto-start RADIUS Server');
            }
        }).catch(err => {
            logger.error('Error auto-starting RADIUS Server:', err);
        });
    }
} catch (error) {
