...
const genieacsApi = {
    ...
    async getDeviceParameters(deviceId, parameters) {
        try {
            console.log('Getting parameters for device:', deviceId, parameters);
            const axiosInstance = getAxiosInstance();
            const device = await axiosInstance.get(`/devices/${encodeURIComponent(deviceId)}`);
            
            const result = {};
            
            // Extract parameter values from device data
            for (const param of parameters) {
                // Try to find the parameter in device data
                const value = this.getNestedValue(device.data, param);
                result[param] = value || 'N/A';
            }
    ...
    async setDeviceParameters(deviceId, parameters) {
        try {
            console.log('Setting parameters for device:', deviceId, parameters);
            const result = await this.setParameterValues(deviceId, parameters);
            return { success: true, data: result };
        } catch (error) {
            console.error(`Error setting parameters for device ${deviceId}:`, error.message);
        ...
        return path.split('.').reduce((current, key) => {
            if (current && typeof current === 'object') {
                ...
                if (current[key] !== undefined) {
                    return current[key]._value !== undefined ? current[key]._value : current[key];
                }
                // Try with _value suffix
                if (current[key + '._value'] !== undefined) {
                    return current[key + '._value'];
                }
            }
            return undefined;
        }, obj);
...
module.exports = {
    ...
    getDeviceParameters: genieacsApi.getDeviceParameters,
    setDeviceParameters: genieacsApi.setDeviceParameters,
