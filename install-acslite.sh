...
echo "[INFO] Configuring Database (Targeting password: $DB_PASS)..."

# Try all known passwords and migrate if successful
OLD_PASSWORDS=("h6Uems6h4HmW1y7" "radius123" "password" "secret")
SUCCESSFUL_PASS=""

# Case 1: No password (fresh install)
if mysql -u root -e "status" &>/dev/null; then
    echo "[INFO] MariaDB has no root password. Setting to '$DB_PASS'..."
    mysql -u root -e "ALTER USER 'root'@'localhost' IDENTIFIED BY '$DB_PASS'; FLUSH PRIVILEGES;"
    SUCCESSFUL_PASS="$DB_PASS"
# Case 2: Already has target password
elif mysql -u root -p"$DB_PASS" -e "status" &>/dev/null; then
    echo "[INFO] Database already using target password '$DB_PASS'."
    SUCCESSFUL_PASS="$DB_PASS"
# Case 3: Try old passwords and migrate
else
    for P in "${OLD_PASSWORDS[@]}"; do
        if mysql -u root -p"$P" -e "status" &>/dev/null; then
            echo "[INFO] Successfully connected using old password '$P'. Migrating to '$DB_PASS'..."
            mysql -u root -p"$P" -e "ALTER USER 'root'@'localhost' IDENTIFIED BY '$DB_PASS'; FLUSH PRIVILEGES;"
            SUCCESSFUL_PASS="$DB_PASS"
            break
        fi
    done
...
if [ -z "$SUCCESSFUL_PASS" ]; then
    echo "[WARNING] Could not determine database password."
    echo "Please ensure password is correct or use 'mysql_secure_installation'."
fi
...
if [ -f "$BINARY_SOURCE" ]; then
    ...
    echo "[INFO] Copied Go binary."
else
    echo "[ERROR] Binary not found at $BINARY_SOURCE"
    exit 1
fi

# Copy entire Web folder recursively
if [ -d "web" ]; then
    echo "[INFO] Copying web files recursively..."
    mkdir -p "$INSTALL_DIR/web"
    cp -rp web/* "$INSTALL_DIR/web/"
    
    # Flatten templates to web/ root for legacy compatibility (so accessible at root /)
    if [ -d "web/templates" ]; then
        cp -rp web/templates/* "$INSTALL_DIR/web/"
        echo "[INFO] Flattened templates to web root."
    fi
    echo "[SUCCESS] Web files structure installation complete."
else
